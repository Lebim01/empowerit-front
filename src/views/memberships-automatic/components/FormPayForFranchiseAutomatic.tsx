...
const FormPayForFranchiseAutomatic = ({
  type,
  createPaymentLink,
  loading,
  period,
  ...
  const address =
    ...
    user.payment_link_automatic_franchises[type]?.address
  const amountcrypto =
    user.payment_link_automatic_franchises &&
    user.payment_link_automatic_franchises[type] &&
    user.payment_link_automatic_franchises[type]?.amount
  const expires_at =
    user.payment_link_automatic_franchises &&
    user.payment_link_automatic_franchises[type] &&
    user.payment_link_automatic_franchises[type]?.expires_at
  const qr =
    user.payment_link_automatic_franchises &&
    user.payment_link_automatic_franchises[type] &&
  ...
  useEffect(() => {
    if (
      ...
      setAmount(
        Number(user.payment_link_automatic_franchises[type].amount) || 0
      )
    }
    ...
    <>
      <div className="flex flex-1 flex-col space-y-2 items-center">
        ...
        {/* <div className="w-full flex justify-end">
          ...
          {user.payment_link_automatic_franchises![type].currency != 'LTC' && (
            <ButtonSwapCurrency
              currency="LTC"
              createPaymentLink={createPaymentLink}
              type={type}
...
export default FormPayForFranchiseAutomatic
