...
export default function CapSlider() {
    ...
    const getLastCycle = async () => {
        ...
        if (user.uid && user.membership) {
            const q = query(collection(db, "users", user.uid, "cycles"), where("type", "==", user.membership), orderBy("created_at", "desc"))
    ...
    const getPayrollAfterLastCycle = async () => {
        ...
        if (user.uid && lastCycle) {
            const q = query(collection(db, "users", user.uid, "payroll"), where("created_at", ">=", lastCycle), orderBy("created_at", "desc"))
            const qSnapshot = await getDocs(q)
    ...
    return (
        <div>
            ...
            <Dialog isOpen={modal} onClose={() => setModal(false)}>
                ...
                <p className="font-semibold">Fecha de adquisición de la franquicia: <span className="font-normal">{lastCycle ? lastCycle.toLocaleDateString() : 'No hay registro'}</span></p>
                {lastCycle &&
                    (dataPayroll && dataPayroll.length > 0 ? (
                        <>
                            ...
                            {dataPayroll.map((data,i) => (
                                <div key={i}>
                                    <p className="font-semibold">{data.created_at.toLocaleDateString()} {'=>'}  <span className="font-normal">${data.total.toFixed(2)}</span></p>
                                </div>
                            ))}
                        </>
