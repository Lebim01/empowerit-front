...
import { getAllParticipations, getAvailableParticipations } from "@/services/Participations"
...
export default function ParticipationsHistory() {
     ...
     const [availableParticipations, setAvailableParticipations] = useState<Participation[] | null>(null)
     ...
     const hasLTCWallet = async (participations: Participation[]) => {
          const updatedParticipations = await Promise.all(participations.map(async (part) => {
               const q = query(collectionGroup(db, 'users'), where("email", "==", part.email), limit(1));
     ...
     const payrollParticipations = async () => {
          try {
               await fetch(
                    `${import.meta.env.VITE_API_URL}/admin/pay-participations`,
                    {
                         method: 'POST',
                         headers: {
          ...
          <div>
               <div className="flex justify-between my-4">
                    ...
                    <Button
                         ...
                         disabled={(availableParticipations?.length == 0 || valuePayroll == 0) ? true : false}
                    >Repartir ${valuePayroll}</Button>
               </div>
               <Table>
                    <THead>
                         <Tr>
                    ...
                    <TBody>
                         {walletsChecked && participations && participations.map((row, index) => (
