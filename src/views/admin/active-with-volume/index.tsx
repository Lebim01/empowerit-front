...
import { AutomaticFranchises, Memberships } from '@/views/memberships/methods'
...
const ActiveWithVolume = () => {
  ...
  const [membership, setMembership] = useState<
    Memberships | AutomaticFranchises
  >('FB79')
  ...
  const enter = async () => {
    ...
    try {
      ...
      if (!res.empty) {
        ...
        toast.push(<Notification title="Usuario activado!" type="success" />, {
          placement: 'top-center',
        })
      } else {
        toast.push(<Notification title="Usuario no existe" type="warning" />, {
          placement: 'top-center',
        })
      }
    } catch (err) {
      ...
      <div className="w-full md:w-1/2 flex flex-col space-y-4 items-end">
        <Select
          ...
          onChange={(e) => {
            ...
            else setMembership('FB79')
          }}
        />
        <Input placeholder="Email" onChange={(e) => setEmail(e.target.value)} />
        <div>
          <Button onClick={enter} loading={loading}>
            Activar
          </Button>
        </div>
      </div>
