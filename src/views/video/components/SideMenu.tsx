...
const SideMenu = () => {
  ...
  const localCourseType = academyType == AcademyType.LEADERSHIP ? CoursesType.LEADERSHIP : CoursesType.STANDARD
  const { data: lessons } = useLessons(localCourseType, courseId)
  const { data: lives } = useLives(localCourseType, courseId)

  console.log({ lessons, lives})
  return (
    <div className="flex flex-col w-full space-y-2">
      {isLive ? lives.map((live) => (
        <Link
          key={live.id}
          to={`/${academyType}/course/${courseId}/live/${live.id}`}
        >
          <div className={classNames("hover:bg-gray-200 px-4 py-3 rounded-md hover:cursor-pointer", { "bg-blue-200": lessonId == live.id })}>
      ...
      lessons.map((lesson) => (
        ...
        >
          <div className={classNames("hover:bg-gray-200 px-4 py-3 rounded-md hover:cursor-pointer", { "bg-blue-200": lessonId == lesson.id })}>
            <p className="line-clamp-1 text-ellipsis font-semibold">
              {lesson.name}
            </p>
            <span className='flex items-center space-x-1'>
