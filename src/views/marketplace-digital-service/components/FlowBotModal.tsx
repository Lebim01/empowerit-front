...
export default function FlowBotModal() {
  ...
  useEffect(() => {
    if (user && user.flow_bot_expires_at) {
      getLeftDays(user.flow_bot_expires_at)
    } else {
      setHasAccess(false)
    }
  }, [user])
  ...
  const buyProcess = async () => {
    ...
    try {
      ...
      console.log('Error en la compra de Flow Bot')
    } finally {
      setOpenModal(false)
      setLoading(false)
  ...
  const createHistoryCreditsDoc = async (total: number) => {
    ...
    await addDoc(collection(db, `users/${user.uid}/credits-history/`), {
      ...
      total,
      created_at: now,
      concept: 'Compra de Acceso de Flow Bot',
      flow_bot_expires_at: expiresAt,
    })
    await addDoc(
      collection(db, `users/${user.uid}/digital-marketplace-purchases/`),
      {
        user_id: user.uid,
        email: user.email,
        purchase: 'Compra de Acceso de Flow Bot',
        cost,
        created_at: new Date(),
      }
    )
  ...
  return (
    <div className="bg-gray-100 flex flex-col items-center rounded-lg px-4 pb-4">
      <img
        src="/img/digital-marketplace/marketing-top-bot.png"
