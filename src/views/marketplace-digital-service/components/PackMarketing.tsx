...
export default function PackMarketing() {
  ...
  const [cost, setCost] = useState<number>(95)
  ...
  useEffect(() => {
    if (user && user.pack_marketing_expires_at) {
      getLeftDays(user.pack_marketing_expires_at)
    } else {
      setHasAccess(false)
    }
  }, [user])
  ...
  const createHistoryCreditsDoc = async (total: number) => {
    ...
    await updateDoc(usersRef, {
      ...
      pack_marketing_expires_at: expiresAt,
    })
    ...
    await addDoc(collection(db, `users/${user.uid}/credits-history/`), {
      ...
      total,
      created_at: now,
      concept: 'Compra de Acceso de Pack Marketing',
      pack_marketing_expires_at: expiresAt,
    })
    await addDoc(
      collection(db, `users/${user.uid}/digital-marketplace-purchases/`),
      {
        user_id: user.uid,
        email: user.email,
        purchase: 'Compra de Acceso de Pack Marketing',
        cost,
        created_at: new Date(),
      }
    )
  ...
  return (
    <div className="bg-gray-100 flex flex-col items-center rounded-lg px-4 pb-4">
      <img
        src="/img/digital-marketplace/marketing-top-pack-amrketing.png"
