...
export default function InstaAdsModal() {
  ...
  const [cost, setCost] = useState<number>(79)
  ...
  useEffect(() => {
    if (user && user.insta_ads_expires_at) {
      getLeftDays(user.insta_ads_expires_at)
    } else {
      setHasAccess(false)
    }
  }, [user])
  ...
  const buyProcess = async () => {
    ...
    try {
      ...
      console.log('Error en la compra de Insta Ads')
    } finally {
      setOpenModal(false)
      setLoading(false)
  ...
  const createHistoryCreditsDoc = async (total: number) => {
    ...
    await updateDoc(usersRef, {
      ...
      insta_ads_expires_at: expiresAt,
    })
    ...
    await addDoc(collection(db, `users/${user.uid}/credits-history/`), {
      ...
      total,
      created_at: now,
      concept: 'Compra de Acceso de Insta Ads',
      insta_ads_expires_at: expiresAt,
    })
    await addDoc(
      collection(db, `users/${user.uid}/digital-marketplace-purchases/`),
      {
        user_id: user.uid,
        email: user.email,
        purchase: 'Compra de Acceso de Insta Ads',
        cost,
        created_at: new Date(),
      }
    )
