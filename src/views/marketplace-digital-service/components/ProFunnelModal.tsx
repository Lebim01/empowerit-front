...
export default function ProFunnelModal() {
  ...
  const [cost, setCost] = useState<number>(49)
  ...
  useEffect(() => {
    if (user && user.pro_funnel_expires_at) {
      getLeftDays(user.pro_funnel_expires_at)
    } else {
      setHasAccess(false)
    }
  }, [user])
  ...
  const createHistoryCreditsDoc = async (total: number) => {
    ...
    await updateDoc(usersRef, {
      ...
      pro_funnel_expires_at: expiresAt,
    })
    ...
    await addDoc(collection(db, `users/${user.uid}/credits-history/`), {
      ...
      total,
      created_at: now,
      concept: 'Compra de Acceso de Pro Funnel',
      pro_funnel_expires_at: expiresAt,
    })
    await addDoc(
      collection(db, `users/${user.uid}/digital-marketplace-purchases/`),
      {
        user_id: user.uid,
        email: user.email,
        purchase: 'Compra de Acceso de Pro Funnel',
        cost,
        created_at: new Date(),
      }
    )
